...
export async function POST(request: Request) {
  ...
  const items = [
    ...
    { productId: byName.get('Snorkeling Kit')?.id, quantity: body.tipo === 'noleggio' ? 0 : body.snorkeling ?? 0 },
    { productId: byName.get('Cooler Box')?.id, quantity: body.tipo === 'noleggio' ? 0 : body.setTramonto ?? 0 },
